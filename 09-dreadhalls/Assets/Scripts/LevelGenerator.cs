...
public class LevelGenerator : MonoBehaviour {
	...
	private int _mazeX = 4, _mazeY = 1;

	// Use this for initialization
	void Start () {

		// initialize map 2D array
		_mapData = GenerateMazeData();
		...
		for (var z = 0; z < mazeSize; z++) {
			for (var x = 0; x < mazeSize; x++) {
				if (_mapData[z, x]) {
					...
					_characterPlaced = true;
				}

				// create floor and ceiling
				if (_mapData[z, x] || Random.Range(1, 100) != 1)
				{
		...
		var myPickup = Instantiate(pickup, new Vector3(_mazeX, 1, _mazeY), Quaternion.identity);
	...
	bool[,] GenerateMazeData() {
		...
		while (tilesConsumed < tilesToRemove) {
			...
			for (int i = 0; i < numSpacesMove; i++) {
				_mazeX = Mathf.Clamp(_mazeX + xDirection, 1, mazeSize - 2);
				_mazeY = Mathf.Clamp(_mazeY + yDirection, 1, mazeSize - 2);

				if (data[_mazeY, _mazeX]) {
					data[_mazeY, _mazeX] = false;
					tilesConsumed++;
				}
			}
		}
