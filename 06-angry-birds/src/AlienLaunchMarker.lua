...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.mouse.wasPressed(1) and not self.split and not self.aliens[1].collided then
            self.split = true
            local bodyX = self.aliens[1].body:getX()
            local bodyY = self.aliens[1].body:getY()
            for i = 1, 2 do

                local alien = Alien(self.world, 'round', bodyX, bodyY, 'Player')
